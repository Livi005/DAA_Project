...
\begin{document}
	...
	\section{Presentación del Problema Original}
	\subsection{Contexto}
	...
	% --- MEJORA AÑADIDA (sin eliminar texto) ---
	Modelamos estos factores reales mediante una \textbf{matriz de costos torre--frecuencia} (o función $w(v,c)$), una \textbf{abstracción estándar en optimización combinatoria} que permite incorporar heterogeneidad económica sin perder la estructura del problema.
	% ------------------------------------------
	...
	% --- MEJORA AÑADIDA: ILP (no implementada), sin tocar tu contenido ---
	\subsection{Formulación alternativa como ILP (no implementada)}
	...
	x_{v,c} =
	\begin{cases}
		1, & \text{si el vértice } v \text{ usa el color } c,\\
		0, & \text{en otro caso.}
	\end{cases}
	\]
	...
	\begin{enumerate}[label=(\alph*)]
		\item \textbf{Asignación única por vértice:} $\sum_{c \in C} x_{v,c} = 1 \quad \forall v \in V$.
		\item \textbf{Interferencia (adyacencia):} $x_{u,c} + x_{v,c} \le 1 \quad \forall \{u,v\}\in E,\ \forall c\in C$.
		\item \textbf{Integralidad:} $x_{v,c} \in \{0,1\} \quad \forall v\in V,\ \forall c\in C$.
	\end{enumerate}
...
% --- MEJORA AÑADIDA: explicación DP completa (estado, subproblema, caso base, recurrencia, exactitud y por qué solo árboles) ---
\textbf{Definición del subproblema (estado DP).} Se fija una raíz arbitraria $r$ y se orientan las aristas implícitamente hacia los hijos. Para cada vértice $v$ y cada color $c \in C$, se define:
\[
...
\textbf{Caso base.} Si $v$ es hoja (no tiene hijos excepto, quizá, su padre en la orientación), entonces el subárbol de $v$ es el propio vértice, y:
\[
dp[v][c] = w(v,c).
\]

\textbf{Recurrencia (paso de programación dinámica).} Si $v$ tiene hijos $u_1,\dots,u_t$, entonces, dado que $f(v)=c$, cada hijo $u_i$ debe elegir un color $d \neq c$. Como los subárboles de distintos hijos son disjuntos en un árbol, sus decisiones pueden optimizarse independientemente y sumarse:
\[
dp[v][c] \;=\; w(v,c) \;+\; \sum_{u \in \text{Hijos}(v)} \min_{d \in C\setminus\{c\}} dp[u][d].
\]

\textbf{Respuesta final.} El costo óptimo del árbol completo es:
\[
\min_{c \in C} dp[r][c].
\]

\textbf{Por qué es exacto.} La exactitud se sigue del \textit{principio de optimalidad}: si fijamos $f(v)=c$, la elección óptima para cada subárbol hijo no depende de decisiones en otros subárboles, porque en un árbol no existen ciclos ni aristas ``cruzadas'' entre subárboles de hijos diferentes. Por tanto, tomar para cada hijo la mejor opción compatible ($d\neq c$) y sumar produce la solución global óptima para el subárbol de $v$.

\textbf{Por qué solo funciona en árboles.} En grafos con ciclos, los ``subproblemas'' ya no son independientes: un vértice puede conectarse con otro que pertenezca a un supuesto ``subárbol'' distinto, y entonces la decisión de color en una parte del grafo restringe simultáneamente múltiples regiones. Esa dependencia global rompe la posibilidad de combinar óptimos locales por simple suma como en la recurrencia anterior.

\textbf{Cálculo de complejidad.} Para cada vértice $v$ y cada color $c$ se calcula una suma sobre sus hijos. El término dominante es el cálculo de
...
% --- MEJORA AÑADIDA: definición explícita de "conflictos" y aclaración de promedios ---
\noindent \textbf{Nota sobre ``conflictos''.} Se entiende por \textit{conflicto} una violación de la restricción de interferencia: una arista $\{u,v\}\in E$ tal que $f(u)=f(v)$. En este trabajo, todas las soluciones reportadas por los algoritmos finales se verifican con \texttt{verificador.py}, por lo que el número de conflictos reportado debe ser 0; cualquier solución con conflictos se considera inválida y no se incluye como solución factible.

\noindent \textbf{Promedios experimentales.} Las métricas de costo y tiempo se reportan como promedios sobre 50 instancias independientes para cada configuración $(n,k)$ y para cada tipo de grafo, evitando mezclar topologías distintas.
% -------------------------------------------------------------------------------
